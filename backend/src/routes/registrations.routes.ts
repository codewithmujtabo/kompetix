...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const insertResult = await pool.query(
      `INSERT INTO registrations (id, user_id, comp_id, status, meta, profile_snapshot)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING registration_number`,
      [id, req.userId, compId, initialStatus, meta ? JSON.stringify(meta) : null, JSON.stringify(profileSnapshot)]
    );
