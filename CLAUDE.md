...
### EMC Port (begins 2026-05-13) — Waves 1–5 ✅ COMPLETE · Wave 6 🚧 IN EXECUTION
...
### SPRINT 25 — EMC Wave 6: The Question Bank (May 16, 2026 — Session 11) 🚧 IN EXECUTION
Wave 6 builds the question bank on the Wave-1 schema: a `question_maker` authoring portal, the subject/topic/subtopic taxonomy, multiple-choice + short-answer questions, and the `proofreads` review workflow. Plan: `/Users/mujtabo/.claude/plans/effervescent-booping-horizon.md`. Decisions: full scope (incl. review); MC + short-answer; single-language content. Phases: (1) authoring API; (2) review API; (3) portal + taxonomy + admin access UI; (4) question editor; (5) review workflow UI.
| Task | What | Key files |
|---|---|---|
| 25.1 | **Phase 1 — question-bank authoring API.** New `question-bank.routes.ts` (mounted `/api`, `requireRole("question_maker")` + a `hasCompAccess` guard reading the `accesses` table): `GET /question-bank/competitions` (the maker's accessible competitions + granted grades); subjects/topics/subtopics CRUD (one generic handler set over the 3 named comp-scoped tables); questions CRUD — list (filter status/subject/grade), get, create, update, soft-delete, writing child `answers` (MC options or the short-answer key) + `question_topics` tags in a transaction, per-comp auto code `Q-NNN`, new questions `status='draft'`, edits writer-only + draft-only. Admin `accesses` endpoints in `admin.routes.ts` (`GET /admin/question-makers`, `GET/POST/DELETE …/question-makers/:userId/accesses` — grant/revoke per-competition access + grades, `audit()`-wrapped). No new migration — the Wave-1 `1748100000000_emc-content` schema is reused. Verified: `tsc` clean; curl — admin grants access, the question_maker lists competitions, builds taxonomy, creates an MC + a short-answer question. Shipped on `feature/wave6-phase1-authoring-api`. | `backend/src/routes/question-bank.routes.ts` (new), `backend/src/routes/admin.routes.ts`, `backend/src/index.ts` |
| 25.2 | **Phase 2 — question-bank review workflow API.** Appended to `question-bank.routes.ts` the editorial state machine `draft → submitted → approved`: `POST …/questions/:id/submit` (writer-only, draft-only), `POST …/approve` (reviewer ≠ writer, submitted-only — sets `approver_id`+`approved_at`, an optional comment becomes a proofread sign-off), `POST …/send-back` (reviewer ≠ writer, submitted-only — **comment required**, returns the question to `draft` + records the comment as a proofread). Proofreads: `GET …/questions/:id/proofreads` (history + reviewer name), `POST` (record a review pass — level/cognitive/answer/comment, `done`→`done_at`; **no self-review** — the writer can't proofread their own question), `PUT …/:proofreadId` (edit own proofread). All `question_maker`-gated + `hasCompAccess`-scoped + `audit()`-wrapped; `answerId`, when given, is validated to be a live option of the question. `loadQuestion` + the questions list also now return `writerName`/`approverName` (the Phase-5 review-queue UI needs them). Verified: `tsc` clean; curl with two question_makers (writer + reviewer) — submit → proofread → approve, and submit → send-back → draft, both round-trip; self-approve + self-proofread both 403; send-back without a comment 400. Shipped on `feature/wave6-phase2-review-api`. | `backend/src/routes/question-bank.routes.ts` |
| 25.3 | **Phase 3 — question_maker portal + taxonomy + admin access UI.** New `(question-maker)` web route group: `questionMakerHttp` client; `question-maker-context.tsx` (`createRoleAuth`, accepts `question_maker`); `(question-maker)/layout.tsx` on the shared `AppShell` (nav: Dashboard / Taxonomy — Questions + Review added in Phases 4–5). New `lib/question-bank/context.tsx` — a `QuestionBankProvider` holding the maker's accessible competitions + the selected one (persisted to `localStorage`) + a shared `<CompetitionPicker>`; the whole portal is scoped to the picked competition. Pages: `/question-bank` dashboard (picker + 4 `StatCard`s — My Drafts/Submitted/Approved + Review Queue, derived from the questions list); `/question-bank/taxonomy` — a 3-column subject → topic → subtopic drill-down, each column inline add / rename (pencil) / delete (trash). Admin: new `/question-makers` page (`(dashboard)` group) — lists question_maker users + access count, a per-maker dialog grants/revokes per-competition access + SD/SMP/SMA grades; "Question Makers" added to the admin sidebar. `destinationFor` routes `question_maker` → `/question-bank`. Verified: `tsc` clean, `npm run build` passes (3 new routes), confirmed in-browser. Shipped on `feature/wave6-phase3-portal-taxonomy`. | `web/lib/api/client.ts`, `web/lib/auth/question-maker-context.tsx` (new), `web/lib/question-bank/context.tsx` (new), `web/app/(question-maker)/*` (new), `web/app/(dashboard)/question-makers/page.tsx` (new), `web/app/{page,(dashboard)/layout}.tsx` |
