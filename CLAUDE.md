...
### EMC Port (begins 2026-05-13) — Waves 1–4 ✅ COMPLETE · Wave 5 🚧 IN EXECUTION
...
### SPRINT 24 — EMC Wave 5: Affiliated Competitions (May 16, 2026 — Session 11) 🚧 IN EXECUTION
Competzy hosts **two competition kinds**: **native** (full lifecycle in-app — Waves 1–4) and **affiliated** (students register + pay on Competzy, then an operator issues each a login + password + an external link; they compete off-platform). Wave 5 builds the affiliated path. Plan: `/Users/mujtabo/.claude/plans/effervescent-booping-horizon.md`. Phases: (1) backend — `kind` flag + credentials + issuance APIs; (2) web — `kind` selector + operator credential-issuing UI; (3) student-facing — external-access step + mobile card.
| Task | What | Key files |
|---|---|---|
