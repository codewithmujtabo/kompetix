...
**Latest milestone — EMC Wave 1 COMPLETE (Phases A + B + C + D):** Sprint 20 shipped end-to-end — unified email/password + phone-OTP login at `/` with role-based auto-routing, full forgot-password flow (`password_reset_tokens` + `/forgot-password` + `/reset-password` pages, 15-min token TTL, single-use, rate-limited), polished register page (inline validation + 8-char password + duplicate-email handling + mobile-responsive split-screen), competition portal generalized to `/competitions/[slug]/{register,dashboard,admin}` driven by a slug→config registry, and the 31-table multi-tenant schema for the question bank + exam delivery + commerce + marketing + config (6 migrations `1748100000000`–`1748600000000`). All user-visible "Eduversal" references stripped — platform is Competzy-only. Ready to plan Wave 2.

### EMC Port (begins 2026-05-13) — Wave 1 ✅ COMPLETE (4 phases shipped)
...
- **Wave 2 catalog URL (locked 2026-05-13 evening):** `/competitions` will be the **public catalog** (student/parent post-login lands here and picks a competition); admin competitions-management page MOVES from `/competitions` to `/admin/competitions` (file: `web/app/(dashboard)/admin/competitions/page.tsx`). Until Wave 2 ships, student/parent post-login goes to `/competitions/emc-2026/dashboard` as a documented placeholder (the default slug in `web/lib/competitions/registry.ts`).
...
- **Run all migrations on VPS:** `cd backend && npm run db:migrate` to apply `1746500000000` through `1748600000000` (covers Sprint 14–17 + the full Sprint 20 batch including the 6 EMC-port migrations).
