...
**Latest milestone — EMC Wave 2 RE-SCOPED to a professional web UI/UX redesign (2026-05-15, Session 11):** after reviewing the running portals the user redirected Wave 2 from feature work to a full UI/UX overhaul — rebuild every web surface (admin, organizer, school, student portal, auth/public pages) on ONE shared design system: Tailwind v4 + shadcn/ui + recharts + lucide-react, a shared `AppShell`, teal/indigo brand, DM fonts — keeping all behavior identical. The original Wave 2 feature scope (catalog, question bank, step-flow) moves to Wave 4+; the mobile re-theme is Wave 3. **Phase 0 (this commit):** the EMC-port role foundation that survives the re-scope — `supervisor` + `question_maker` roles (`1748700000000_emc-roles`), `requireRole()` middleware, `compFilter()` helper + extended soft-delete whitelist, `create-question-maker.ts` / `create-supervisor.ts` seed scripts. Wave 2 plan file: `/Users/mujtabo/.claude/plans/synthetic-dreaming-boot.md`.
