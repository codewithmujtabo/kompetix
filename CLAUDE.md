...
### SPRINT 18 — English UI Pass & Back-Nav Consistency (May 11, 2026 Session 6) ✅ COMPLETE
| Task | What | Key files |
|---|---|---|
| 18.1 | **Remove remaining Indonesian strings** — "Bergabung" → "Joined", "About Kompetisi" → "About Competition", "Kategori" → "Category"/"Categories", "Round & Jadwal" → "Round & Schedule", "Lokasi/Platform" → "Location/Platform", "GRADE KELAS" / "GRADE KELAS ANAK" → "GRADE" / "CHILD GRADE", `compName ?? "Kompetisi"` fallback → "Competition". Mobile UI is now English-only. | `app/app/(tabs)/my-registration-details.tsx`, `app/app/(tabs)/competitions/[id].tsx`, `app/app/(tabs)/competitions.tsx`, `app/app/(tabs)/profile/history.tsx`, `app/app/(tabs)/my-competitions.tsx`, `app/app/(auth)/register.tsx` |
| 18.2 | **Date locale `id-ID` → `en-US`** for all date formatters (6 files). Months render as "May 11, 2026" instead of "11 Mei 2026 pukul 13.45". Currency `Rp X.XXX` left as `id-ID` because the period thousand separator is the standard Rupiah convention. | `app/app/(tabs)/my-registration-details.tsx`, `app/app/(tabs)/competitions/[id].tsx`, `app/app/(tabs)/competitions.tsx`, `app/app/(tabs)/notifications.tsx`, `app/app/(tabs)/profile/document-vault.tsx` |
| 18.3 | **Back-nav from competition detail respects source tab.** `competitions/[id].tsx` is registered as a `Tabs.Screen` under the `competitions` (Discover) segment, so pushing to it from another tab implicitly switched the active tab to Discover — `router.back()` then landed on Discover instead of the source. Fix: detail screen reads a `from` query param and routes back to `/(tabs)/my-competitions`, `/(tabs)/notifications`, or `/(tabs)/profile/history` explicitly. Callers (`my-competitions.tsx` ×4, `notifications.tsx` ×1) now pass `from: "..."` on push. Discover pushes don't need it. | `app/app/(tabs)/competitions/[id].tsx`, `app/app/(tabs)/my-competitions.tsx`, `app/app/(tabs)/notifications.tsx` |
| 18.4 | **Unify back-button affordance.** Removed text-link "← Back" headers in favor of the circular-chevron `ScreenHeader` everywhere. 4 screens updated; their old `backButton`/`backButtonText`/`title` styles dropped along with the `+60px` top padding hack on `edit.tsx`. | `app/app/(tabs)/profile/edit.tsx`, `app/app/(tabs)/profile/document-vault.tsx`, `app/app/(tabs)/teacher-actions.tsx`, `app/app/(auth)/register.tsx` |
| 18.5 | **My Competitions tab bar polish.** Removed emoji icons (💙 📝 🏅), switched to pill-shaped segmented control with circular count badges; inactive badge bg bumped from `Brand.primarySoft` (#EEF2FF, washed out) to `#DBE2FE` for readable contrast against the active filled badge. | `app/app/(tabs)/my-competitions.tsx` |
