...
- Tables: `users`, `students`, `parents`, `teachers`, `competitions`, `competition_rounds`, `registrations`, `payments`, `documents`, `notifications`, `otp_codes`, `invitations`, `parent_student_links`, `teacher_student_links`, `bulk_registration_jobs`, `favorites`, `schools`, `historical_participants`, `school_payment_batches`, `school_payment_batch_items`, `organizers`, **`audit_log` (Sprint 14)**, **`payment_webhook_events` (Sprint 14)**, **`password_reset_tokens` (Sprint 20 Phase B)**.
...
- **Latest migration:** `1748050000000_password-reset-tokens.sql` (Sprint 20 Phase B). Sprint 20 batch also adds: `1748000000000_add-competition-slug` (de-branded seed), `1748010000000_debrand-existing-emc-row` (renames locally-seeded Eduversal row). Earlier Sprint 14–17 batch: `1746500000000_teacher-student-links`, `1746800000000_rebrand-registration-prefix-to-CTZ`, `1746900000000_audit-log`, `1747000000000_soft-delete`, `1747100000000_payment-webhook-events`, `1747200000000_add-person-kid`, `1747300000000_drop-orphan-parent-school-id`, `1747400000000_add-payer-user-id`, `1747500000000_school-verification`, `1747600000000_promote-legacy-approved-registrations`. Run all on VPS.
...
- `web/app/page.tsx` — **Unified login** (Sprint 20): split-screen branded, email OR phone-OTP mode toggle, session detection ("Already signed in" chooser), role-based auto-route, light/dark toggle, forgot-password link, footer (Privacy / Terms / Contact).
...
**Latest milestone (Phases A + B + C of EMC Wave 1):** Sprint 20 shipped — unified email/password + phone-OTP login at `/` with role-based auto-routing, full forgot-password flow (`password_reset_tokens` + `/forgot-password` + `/reset-password` pages, 15-min token TTL, single-use, rate-limited), polished register page (inline validation + 8-char password + duplicate-email handling + mobile-responsive split-screen). Competition portal generalized — `/emc/{register,dashboard,admin}` retired in favor of `/competitions/[slug]/{register,dashboard,admin}` driven by a slug→config registry (`web/lib/competitions/registry.ts`). All user-visible "Eduversal" references stripped — platform is Competzy-only.
...
- **Wave 1 phases:** (A) cleanup + de-brand to Competzy-only ✅ shipped; (B) login + register polish (forgot-password, phone OTP, mobile-responsive) ✅ shipped; (C) generalize `/emc/*` → `/competitions/[slug]/*` ✅ shipped; (D) 6 new migrations creating 31 multi-tenant tables — NEXT.
...
- **Run all migrations on VPS:** `cd backend && npm run db:migrate` to apply `1746500000000` through `1748050000000` (covers Sprint 14–17 + Sprint 20 phases A & B), and after Wave 1 Phase D runs locally also `1748100000000`–`1748600000000`.
...
### SPRINT 20 — Unified Login + De-Brand + Production-Quality Auth + Slug-Based Portal Routes (May 13, 2026 Session 10) — Phases A, B & C of EMC Wave 1 ✅ COMPLETE
| Task | What | Key files |
|---|---|---|
...
- **Competition portal routes are slug-keyed** at `/competitions/[slug]/{register,dashboard,admin}` (Sprint 20 Phase C). To add a new competition (ISPO, OSEBI, …): (1) add a row to `competitions` with a unique `slug`, (2) add a `CompetitionPortalConfig` entry to `web/lib/competitions/registry.ts` keyed by that slug. No new route files needed. Default student/parent post-login landing is `/competitions/emc-2026/dashboard` (the slug is `DEFAULT_COMPETITION_SLUG`); Wave 2 replaces this with a real `/competitions` catalog page.
